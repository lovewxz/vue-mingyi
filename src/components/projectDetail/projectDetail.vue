<template>
  <transition name="moveInLeft">
    <div class="project-detail">
      <scroll ref="projectDetailScroll"
              class="project-detail-scroll">
        <div class="project-wrapper"
             v-if="projectDetail">
          <div class="project-swiper-wrapper">
            <swiper :options="swiperOption"
                    class="project-swiper">
              <swiper-slide v-for="item in projectDetail.detail_images"
                            class="swiper-item"
                            :key="item">
                <img :src="cdnName(item,400)"
                     @load="loadImage">
              </swiper-slide>
            ...
            <i class="iconfont icon-home"
               @click="backHome"></i>
            <i class="iconfont"
               :class="isFavorite ? 'icon-star-full' : 'icon-star'"
               @click="favor"></i>
          </div>
          <div class="project-content">
            ...
            <div class="price">
              <i>¥</i>
              <span class="num">{{projectDetail.price}}</span>
          ...
          <div class="project-doctor"
               v-if="projectDetail.doctor"
               @click="goDoctor(projectDetail.doctor._id)">
            <div class="doctor-img">
              <img v-lazy="cdnName(projectDetail.doctor.avatar,120)"
                   alt="">
            </div>
          ...
          <div class="project-service">
            ...
            <div class="service-detail">
              ...
              <ul v-if="projectDetail.params">
                <li v-for="(item,index) in projectDetail.params"
                    :key="index">
      ...
      <div class="bottom-bar">
        <ul>
          ...
          <li class="size-s">
            <i class="iconfont"
               :class="isFavorite ? 'icon-star-full' : 'icon-star'"></i>
            <span class="text">收藏</span>
          </li>
          <li class="zx-wrapper">
            <span class="btn-zx">在线咨询</span>
          </li>
          <li class="buy-wrapper"
              @click.stop="showMask">
            <span class="btn-buy">立即预订</span>
          </li>
        </ul>
      </div>
      <project-mask ref="mask"
                    :price="parseInt(projectDetail.price)"
                    :cover-img="projectDetail.cover_image"
                    @confirm="confirm"></project-mask>
      <transition name="moveInLeft">
...
export default {
  ...
  computed: {
    sale() {
      return (
        Number(this.projectDetail.price) /
        Number(this.projectDetail.original_price) *
        10
      ).toFixed(1)
    },
    isFavorite() {
  ...
  methods: {
    ...
    favor() {
      ...
      if (!this.user) {
        this.$router.push({
          name: 'login',
          query: { visit: name, id: this.projectDetail._id }
        })
        return
      }
    ...
    confirm(params) {
      params = Object.assign({}, params, {
        coverImg: this.projectDetail.cover_image[0],
