<template>
  <div class="case-list"
    ...
    <scroll :listen-scroll="listenScroll"
      ...
      <div>
        ...
        <div class="case-project"
          ...
          <div class="case-top">
            <span class="head-img"
                  v-if="caseData.compare_photo">
              <img v-lazy="cdnName(caseData.compare_photo.after,80)"
                   alt="">
            </span>
            <span class="head-name">{{caseData.user_name}}</span>
          </div>
          <div class="case-project-detail"
               @click="goUrl(caseData.project._id)"
               v-if="caseData.project">
            <div class="cover-img">
              <img v-lazy="cdnName(caseData.project.cover_image,140)"
                   alt="">
            </div>
          ...
          <div class="tags">
            <ul>
              <li v-for="(item,index) in caseData.all_item"
                  class="tag-name"
                  :key="index">
                {{item}}
              </li>
        ...
        <div class="case-before">
          ...
          <div class="before-imgs"
               v-if="caseData.compare_photo">
            <ul>
              <li class="before-item">
                <img v-lazy="cdnName(caseData.compare_photo.before,180)"
                     alt="">
              </li>
              <li class="before-item">
                <img v-lazy="cdnName(caseData.compare_photo.before,180)"
            ...
            <li v-for="diary in diaryData"
              ...
              <router-link :to="`/case/list/${caseData._id}/${diary._id}`">
                ...
                <div class="section-imgs">
                  <ul>
                    <li v-for="(img,index) in diary.images"
                        :key="index"
                        class="img-item">
                      <img v-lazy="cdnName(img,300)"
                           alt="">
                    </li>
                  </ul>
                </div>
    ...
    <layer :case-data="caseData"
           ref="layer"></layer>
    <fix-right @back="back"
               @backTop="backTop"
               ref="fixRight"></fix-right>
...
export default {
  ...
  watch: {
    scrollY(newVal) {
      ...
      Math.abs(newVal) > winHeight
        ? this.$refs.fixRight.show()
        : this.$refs.fixRight.hide()
    },
    async $route(to, from) {
      const reg = /\/case\/list\//
      if (reg.test(to.path)) {
