...
export const getYMDoctor = async(page = 1) => {
  ...
  writeFileSync(r('../database/json/doctor.json'), JSON.stringify(doctor, null, 2), 'utf-8')
  ...
  const getProjects = async(page = 1) => {
    ...
    if (count > projects.length) {
      ...
      $('a.list-a').each(function () {
        const projectId = $(this).attr('href').match(/\/([0-9]+)\/$/)[1]
  ...
  const getCases = async(page = 1) => {
    if (count > cases.length) {
      ...
      $('a.boxItem2').each(function () {
        const caseId = $(this).attr('href').match(/\/([0-9]+).html$/)[1]
...
export const checkYMDoctor = () => {
  ...
  writeFileSync(r('../database/json/checkDoctor.json'), JSON.stringify(newData, null, 2), 'utf-8')
}
...
export const uploadYMDoctor = async() => {
  ...
  writeFileSync(r('../database/json/finalYMDoctor.json'), JSON.stringify(data, null, 2), 'utf-8')
}
