    ...
    <el-form :model="diaryData"
      ...
      <sticky>
        <template v-if="fetchSuccess">
          ...
          <el-button v-loading="loading"
                     type="warning"
                     @click="diaryCancel">取消</el-button>
        </template>
        <template v-else>
      ...
      <div class="createPost-main-container">
        <el-form-item label="恢复天数"
                      prop="title">
          <el-col :span="5">
            <el-input v-model="diaryData.title"
                      placeholder="请填写天数"></el-input>
          </el-col>
        </el-form-item>
        <el-form-item label="选择案例"
                      prop="caseId">
          <el-select placeholder="请选择案例"
                     v-model="diaryData.caseId">
            <el-option v-for="item in pcase"
                       :key="item._id"
        ...
        <el-form-item class="editor-content-item"
                      label="文章详情"
                      prop="article"
                      label-width="80px">
          <quil-editor :content="diaryData.article"
                       @articleChange="articleChange"></quil-editor>
        </el-form-item>
      </div>
    </el-form>
  </div>
...
export default {
  data() {
    return {
      ...
      formRules: {
        title: [{ required: true, message: '请输入天数', trigger: 'blur' }],
        caseId: [{ required: true, message: '请输入内容', trigger: 'change' }],
        article: [{ required: true, message: '请输入内容', trigger: 'blur' }]
      },
      pcase: [],
      fetchSuccess: true,
      loading: false
