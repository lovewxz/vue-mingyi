...
export default {
  ...
  methods: {
    ...
    _saveResult(data) {
      ...
      _data.cover_image = _data.cover_image.map(item =>
        item.url.replace(`${config.imgCDN}/`, '')
      )
      _data.detail_images = _data.detail_images.map(item =>
        item.url.replace(`${config.imgCDN}/`, '')
      )
      return _data
    },
    _genResult(data) {
      if (data.cover_image.length > 0) {
        const newImgs = []
        data.cover_image.forEach(item =>
          newImgs.push({ name: item, url: `${config.imgCDN}/${item}` })
        )
        data.cover_image = newImgs
      }
      if (data.detail_images.length > 0) {
        const newImgs = []
        data.detail_images.forEach(item =>
          newImgs.push({ name: item, url: `${config.imgCDN}/${item}` })
        )
